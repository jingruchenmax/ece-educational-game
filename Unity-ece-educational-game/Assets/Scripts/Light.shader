	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput vert(VertexInput input)
			{
				...
				output.intensity = _IntensityFactor * _ContrastFactor;
		...
		Pass
		{
			...
			fixed4 frag(VertexOutput input) : COLOR
			{
				...
				fixed4 finalColor;
				finalColor = diffuseColor*_LightIntensity + (partColor + part2Color) * _ParticleFactor* clamp(0,1,_LightIntensity*5);
